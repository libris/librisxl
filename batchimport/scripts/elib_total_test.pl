...
chdir "/appl/import2/queues/elib" || die "FXXF";
...
for my $product ( @{$products->{'Products'}} ) {
	if ( defined $holdings{$product->{'ProductId'}} ) {
...
eval {
	open(my $fh, '>:encoding(UTF-8)', './elib_testar.json');
...
eval {
	open(my $fh, '>:encoding(UTF-8)', './elib_testar.xml');
...
if ( $@ ) {
	print "elib: write of ./elib.xml failed.";
}
...
for my $p ( keys %$D ) {
	if ( $D->{$p}->{'a_cs'} == $D->{$p}->{'b_cs'} ) {
		...
		$position = $D->{$p}->{'a_pos'};
		# diff holdings
		if ( ! defined $current->{'Products'}->[$position]->{'Holdings'} ) { $current->{'Products'}->[$position]->{'Holdings'} = []; }
