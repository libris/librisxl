...
DEST=/appl/import/queues/ur/tmp
DATE=`date "+%Y-%m-%d"`
...
# /appl/import/queues/$CUST/prev/$FILE.prev får inte tagas bort!!!
if [ -f /appl/import/queues/$CUST/prev/$FILE.prev ]; then
	PREV="/appl/import/queues/$CUST/prev/$FILE.prev"
else
	rm -f /appl/import/queues/$CUST/prev/$FILE.0	
	touch /appl/import/queues/$CUST/prev/$FILE.0
	PREV="/appl/import/queues/$CUST/prev/$FILE.0"
fi

rm -f $DEST/$FILE.diff

/appl/import/scripts/ur_diff.pl $PREV $DEST/$FILE $DEST/$FILE.diff

if [ $? -eq 0 ]; then
	rm -f /appl/import/queues/$CUST/prev/$FILE.prev
	cp -f $DEST/$FILE /appl/import/queues/$CUST/prev/$FILE.prev
...
/appl/import/scripts/ur_split.pl $DEST/$FILE
if [ $? -ne 0 ]; then
...
mv $tmpfile /appl/import/queues/$CUST/incoming/$CUST.$DATE.$tmps.xml
#mv $tmpfile /appl/import/queues/$CUST/$CUST.$DATE.$tmps.xml
