    ...
    <xsl:template match="ims:manifest[@identifier != 'root_manifest']/ims:metadata/ims2:lom">
        ...
        <xsl:variable name="cond-deal">
            <xsl:for-each select="ims2:general/ims3:distributionevents/ims3:distributionevent">
                ...
                <!--<xsl:if test="$case-platform = 'internet' and $case-type = 'download' and $case-rag = 'avc' and ((contains($startDate, 'T') and $timeStamp >= $startDate) or (contains($endDate, 'T') and $timeStamp &lt;= $endDate))"><xsl:text>OK</xsl:text></xsl:if>-->
             </xsl:for-each>
        </xsl:variable>
        ...
        <xsl:if test="$title != '' and $cond-deal != '' and $case-status = 'final' and $case-learningresourcetype = 'program' and ($case-format = 'video' or $case-format = 'audio')">
            ...
            <record type="Bibliographic">
                ...
                <xsl:variable name="pos18-34">
                    <xsl:choose>
                        <xsl:when test="$case-format='audio'"><xsl:text>||| c ||||||     </xsl:text></xsl:when>
                        <xsl:when test="$case-format='video'"><xsl:text>||| c     |    v|</xsl:text></xsl:when>
                ...
                <datafield tag="046" ind1="" ind2="">
                    ...
                    <xsl:if test="substring-before($cond-deal, '|') != ''"><subfield code="m"><xsl:value-of select="substring-before($cond-deal, '|')"/></subfield></xsl:if>
                    <xsl:if test="substring-after($cond-deal, '|') != ''"><subfield code="n"><xsl:value-of select="substring-after($cond-deal, '|')"/></subfield></xsl:if>
                </datafield>
                ...
                <xsl:for-each select="ims2:general/ims3:maintitle[position() > 1]">
                    <datafield tag="246" ind1="1" ind2="1">
                        ...
                        <subfield code="a"><xsl:value-of select="."/><xsl:if test="$remainderoftitle"><xsl:text> : </xsl:text><xsl:value-of select="$remainderoftitle"/></xsl:if></subfield>
                    </datafield>
                </xsl:for-each>
                ...
                <datafield tag="856" ind1="4" ind2="8">
                    ...
                    <subfield code="z">Mediebiblioteket</subfield>
            ...
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Mde'"/><xsl:with-param name="proxy" select="'http://ep.bib.mdh.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Tillgänglig inom högskolan och hemifrån via proxy-server för användare vid Mälardalens högskola'"/></xsl:call-template>
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Mdv'"/><xsl:with-param name="proxy" select="'http://ep.bib.mdh.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Tillgänglig inom högskolan och hemifrån via proxy-server för användare vid Mälardalens högskola'"/></xsl:call-template>
            
            <!-- Göteborgs universitet -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Gdig'"/><xsl:with-param name="proxy" select="'http://ezproxy.ub.gu.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Extern access endast forskare och studenter vid GU'"/></xsl:call-template>       
            
            <!-- Högskolan i Gävle -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Hig'"/><xsl:with-param name="proxy" select="'http://webproxy.student.hig.se:2048/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Tillgänglig för studenter och anställda vid Högskolan i Gävle'"/></xsl:call-template>
            
            <!-- Högskolan Kristianstad -->  
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Krh'"/><xsl:with-param name="proxy" select="'https://ezproxy.bibl.hkr.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Tillgänglig inom Högskolan Kristianstad'"/></xsl:call-template>
            
            <!-- Luleå tekniska universitet -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'La'"/><xsl:with-param name="proxy" select="'http://proxy.lib.ltu.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'(Endast för användare inom LTU)'"/></xsl:call-template>       
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Lam'"/><xsl:with-param name="proxy" select="'http://proxy.lib.ltu.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'(Endast för användare inom LTU)'"/></xsl:call-template>       
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Lat'"/><xsl:with-param name="proxy" select="'http://proxy.lib.ltu.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'(Endast för användare inom LTU)'"/></xsl:call-template>       
            
            <!-- Högskolan Väst -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Htu'"/><xsl:with-param name="proxy" select="'http://ezproxy.server.hv.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Tillgänglig för studenter och anställda vid Högskolan Väst'"/></xsl:call-template>
            
            <!-- Högskolan i Borås -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Hib'"/><xsl:with-param name="proxy" select="'http://costello.pub.hb.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'TV- och radioprogram'"/></xsl:call-template>       
            
            <!-- Högskolan i Kalmar -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Hkal'"/><xsl:with-param name="proxy" select="'http://proxy.hik.se:2048/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Tillgänglig inom högskolan och externt via proxy-server för användare vid HIK'"/></xsl:call-template>       
            
            <!-- Blekinge tekniska högskola -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Bth'"/><xsl:with-param name="proxy" select="'http://miman.bib.bth.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Online access for BTH'"/></xsl:call-template>
            
            <!-- Mittuniversitetet -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Mh'"/><xsl:with-param name="proxy" select="'http://www.bib.miun.se/php/go.php?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'UR access. Endast tillgänglig för Mittuniversitetets forskare och studenter. (begränsad åtkomst)'"/></xsl:call-template>
            
            <!-- Uppsala Learning Lab -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Udig'"/><xsl:with-param name="proxy" select="'http://ezproxy.its.uu.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Tillgänglig för användare inom Uppsala universitet'"/></xsl:call-template>       
            
            <!-- Växjö universitet -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Vo'"/><xsl:with-param name="proxy" select="'http://databas.bib.vxu.se:2048/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Tillgänglig för studenter och anställda vid Växjö universitet'"/></xsl:call-template>
            
            <!-- Karlstads universitet -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Kd'"/><xsl:with-param name="proxy" select="'http://0-uraccess.navicast.net.biblos.kau.se/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'UR Access'"/></xsl:call-template>       
            
            <!-- Högskolan i Jönköping -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Jon'"/><xsl:with-param name="proxy" select="'http://login.bibl.proxy.hj.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'UR access (Endast tillgänglig för forskare och studenter vid Högskolan i Jönköping)'"/></xsl:call-template>       
            
            <!-- Linköpings universitet -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Li'"/><xsl:with-param name="proxy" select="'http://lt.ltag.bibl.liu.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Extern access endast anställda och studenter vid Liu'"/></xsl:call-template>       
            
            <!-- Örebro universitet -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'O'"/><xsl:with-param name="proxy" select="'http://db.ub.oru.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Tillgänglig inom Örebro universitet och hemifrån via proxyserver'"/></xsl:call-template>       
            
            <!-- Lunds universitet -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Ldig'"/><xsl:with-param name="proxy" select="'http://ludwig.lub.lu.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'(Endast för användare inom LU)'"/></xsl:call-template>
            
            <!-- Högskolan på Gotland -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Hgot'"/><xsl:with-param name="proxy" select="'http://proxy.hgo.se:2048/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Tillgänglig inom Almedalsbiblioteket och Högskolan på Gotland och externt för användare vid Högskolan'"/></xsl:call-template>       
            ...
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'H'"/><xsl:with-param name="proxy" select="'https://www04.sub.su.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Endast tillgänglig inom SU:s nät'"/></xsl:call-template>
            
            <!-- Högskolan i Halmstad -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Hal'"/><xsl:with-param name="proxy" select="'http://ezproxy.bib.hh.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Extern access via proxy-server endast för anställda och studenter vid Högskolan i Halmstad'"/></xsl:call-template>
            
            <!-- Högskolan i Skövde -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Skov'"/><xsl:with-param name="proxy" select="'https://persefone.his.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Tillgänglig för användare inom Högskolan i Skövde'"/></xsl:call-template>
            
            <!-- Södertörns högskola -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'D'"/><xsl:with-param name="proxy" select="'http://bibl.sh.se/infosok/ebokip.asp?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Access for students/staff at Södertörns högskola, SH'"/></xsl:call-template>

            <!-- Malmö högskola -->
            <xsl:call-template name="create-holding"><xsl:with-param name="interval" select="$cond-deal"/><xsl:with-param name="sigel" select="'Mah'"/><xsl:with-param name="proxy" select="'http://support.mah.se/login?url=http://uraccess.navicast.net/mov.php?xid='"/><xsl:with-param name="id" select="$id"/><xsl:with-param name="comment" select="'Tillgänglig för studenter och anställda vid Malmö högskola'"/></xsl:call-template>
        </xsl:if>
    </xsl:template>
