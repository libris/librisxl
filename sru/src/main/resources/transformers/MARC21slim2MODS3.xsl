...
-->
	...
	<xsl:template name="marcRecord">
		...
		<!-- authority svep 20160929 KP -->
		<xsl:for-each select="marc:datafield[@tag='072' and marc:subfield[@code = '2' and . = 'swepub-publicationtype']]/marc:subfield[@code='a']">
			<genre authority="svep" type="publicationType"><xsl:value-of select="."/></genre>
		</xsl:for-each>
		<xsl:for-each select="marc:datafield[@tag='072' and marc:subfield[@code = '2' and . = 'swepub-contenttype']]/marc:subfield[@code='a']">
			<genre authority="svep" type="contentType"><xsl:value-of select="."/></genre>
		</xsl:for-each>
...
<!--			<xsl:variable name="langCodes">			
			...
			<xsl:choose>
				<xsl:when test="marc:subfield[@code='2']='rfc3066'"><!-- not stacked but could be repeated -->
		...
		<xsl:for-each select="marc:datafield[@tag=024][@ind1=1]">
			<identifier type="upc">
                ...
                <!-- 2009-06-06 MM - lägg till gamla LIBRIS-nummer som identifierare  -->
		<xsl:for-each select="marc:datafield[@tag=035]/marc:subfield[@code = '9'][starts-with(., '99')]">
                    <identifier type="libris99">
                        <xsl:value-of select="."/>
                    </identifier>
		</xsl:for-each>
