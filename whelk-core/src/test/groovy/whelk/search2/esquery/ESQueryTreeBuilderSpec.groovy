...
class ESQueryTreeBuilderSpec extends Specification {
    ...
    def "match all if empty"() {
        ...
        Node emptyTree = QueryTree.newEmpty().tree()
        Map result = ESQueryTreeBuilder.buildFrom(emptyTree, esSettings).dsl()
    ...
    def "ES query: nested (CONDITION, multi-token value, composite property)"() {
        given:
        ESSettings esSettings = new ESSettings(esMappings, new ESBoost([:]))
        String q = 'p3.p16:(x y)'
        Node expandedQueryTree = new QueryTree(q, disambiguate).expand(jsonLd).tree()
        Map result = ESQueryTreeBuilder.buildFrom(expandedQueryTree, esSettings).dsl()
    ...
    def "like query"() {
        ...
        Condition likeCondition = (Condition) QueryTreeBuilder.buildTree(q, disambiguate)
        ((Link) likeCondition.value()).setSearchNeedle("a b c")
        Map<String, Object> result = ESQueryTreeBuilder.buildFrom(likeCondition, esSettings).dsl()
