...
class JsonLD2MarcXMLConverter implements FormatConverter {
    ...
    static MarcRecord prepareRecord(MarcRecord record, String identifier, String modified) {
        ...
        for (field in record.getDatafields("887")) {
            if (!field.getSubfields("2").isEmpty() && field.getSubfields("2").first().data == "librisxl") {
                has887Field = true
                Subfield subFieldA = field.getSubfields("a").first()
                subFieldA.setData(mapper.writeValueAsString(["@id":identifier, "modified":modified]))
            }
        }
        if (!has887Field) {
            Datafield df = record.createDatafield("887")
