...
class AuthBibMatcherSpec extends Specification {
    ...
    def "Has valid authRecords"() {
        ...
        then:
        AuthBibMatcher.hasValidAuthRecords(authRecords)
    }
    def "Has invalid authRecords"() {
        when:
        def authRecords = [
                ['bibid': 2, 'id': 40578, 'data': ['leader': '00229cz  a2200097n  4500', 'fields': [['181': ['ind1': '1', 'ind2': ' ', 'subfields': [['a': 'Bertilsson, Bertil']]]]]]]
        ]
        then:
        !AuthBibMatcher.hasValidAuthRecords(authRecords)
    }
