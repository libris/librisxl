...
def where = """
  collection = 'bib' 
  AND (data#>>'{@graph, 1, instanceOf, summary}' like '%ormgivare:%' OR data#>>'{@graph, 1, summary}' like '%ormgivare:%')
  AND deleted = false
  """
...
selectBySqlWhere(where) { bib ->
    ...
    def coverDesigners = workContribution.findAll {
        def a = it.role && ROLES.values().containsAll(asList(it.role)*.'@id')
        def b = nameToRoles.containsKey(name(it.agent)) && (asList(it.role) == OTHER || !it.role)
        a || b
    }
    ...
    coverDesigners.each {
        if (asList(it.role) == OTHER || !it.role) {
            it['role'] = nameToRoles[name(it.agent)].collect { ['@id': it] }
        }
    }
