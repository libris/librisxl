...
selectBySqlWhere(
        "data#>'{@graph,1,instanceOf,subject}' @> [{\"@type\":\"ComplexSubject\"}] OR " +
        "data#>'{@graph,1,subject}' @> [{\"@type\":\"ComplexSubject\"}]",
        silent: false, { bib ->
    scheduledForUpdate.println("${bib.doc.getURI()}")

    boolean changed = false

    if (bib.graph[1].instanceOf.subject != null && bib.graph[1].instanceOf.subject instanceof List) {
